...
### Keep the decision lens out of research

Superposition collapses too early not only by picking the first plausible answer, but by filtering options through the decision-maker's constraints *during* research. Evaluate options on their own merits up to SYNTHESIS_GATE; apply fit / profile / recommendation only **after** it. Premature filtering under the goal-owner's constraints is a classic failure — the options that don't obviously fit never get discovered.
...
| Agent computes numbers in prose | Route every calculation through the calculator / JS-sandbox tool |

---

## Constraints Block (optional)

When the task has hard boundaries, add after `<guide>`:
