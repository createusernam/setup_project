...
- Настроенная long-context модель будет читать 50k+ токенов кода; XML-маркеры помогают sparse attention
...
| Bootstrap проекта | `/startup` + discovery | — | Не спрашивать stack/modules до brief и plan |
| Архитектурный handoff (Phase 2b) | `/planning-with-files` + `/pm-review` | `/grace-init` → `/grace-plan` | Инициализировать GRACE из approved brief/plan; technology decisions разрешить в grace-plan |
...
| Реализация | `/build-loop` или `/tdd` по выбранному route | — | Не создавать конкурирующий execution policy внутри bridge |
...
1. Завершить discovery, `/grill-with-docs`, `/planning-with-files` и `/pm-review`.
2. По evidence из plan/risk route решить, требуется ли Phase 2b.
3. `/grace-init` — скаффолд XML из approved artifacts, сохраняя нерешённые technology fields как `pending`.
4. `/grace-plan` — выбрать stack, модули, контракты, зависимости и verification surfaces с named technical reviewers.
5. `/grace-verification` — углубить log-driven trace markers для критичных переходов состояния.
6. Продолжить канонический machine route: design/contract → visualization → issues → scaffold → `/build-loop` или `/tdd`.
