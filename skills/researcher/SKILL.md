---
name: researcher
description: Multi-agent research flow with parallel workers, consensus, synthesis, and judge validation. Modes: market, technical, user, domain, custom (custom is the extension point — domain-specific applications ride on it rather than adding a mode). Invoke when user says "research", "investigate", "discover", "analyze market", or product_brief.md has gaps.
---
...
When the synthesis will become a human-readable deliverable (not just a state file), fill a fixed 6-section scaffold instead of a free-form narrative. It forces completeness — you can't quietly skip a contradiction or drop the question-bank. For full formatting rules per genre, see [Deliverable Style Guide](#deliverable-style-guide).
...
**Apply when**: a human will read the output to understand or decide — market overview, research memo, strategy doc, **or a teaching guide / field textbook**.
**Skip when**: the consumer is another agent or a state file — keep the JSON.

Related: the [synthesis scaffold](#synthesis-scaffold--for-a-long-shareable-synthesis-optional) in Phase 3 (6-section structure for when synthesis IS the deliverable) and the [intent visualization checkpoint](#intent-visualization-checkpoint) in Phase 4 (Mermaid diagram requirement before judge).

### Core law

A concept is introduced by the **pain** that forced its invention — "understand the pain and the fix makes sense forever; memorize the fix and you forget it by Friday." Lead every unit with its pain, never its definition. That single move is what separates a guide people finish from a reference they abandon.

### Genre selection

Pick the genre first — it fixes three things at once: where the through-line lands, the per-unit *loop*, and the *readout*. Don't mix genres inside one document.

| Genre | Unit | Per-unit loop | Readout | Through-line lands at |
|-------|------|---------------|---------|-----------------------|
| **options / opportunity** — market map, niches, delivery models | an option | proof it's real → replicable example → local reality → boundary | viability-strip: `effort · payoff · channel · verdict` | where the openings are |
| **knowledge / explainer** — teaching guide, field textbook, concept deep-dive | a concept / chapter | pain (why it exists) → how it works → diagram → good practices → where it breaks → what they'll ask | verbatim-Q&A: `> Спросить дословно: "…"` + model answer | the so-what / what to do with it |
| **decision** — recommend one path | a candidate | context → option → tradeoff → why-not | next-action: what to do Monday | the recommendation |

### Universal writing rules

These apply to every genre — a structure a reader can follow and scan:

1. **One through-line, not a pile of sections.** The report is a story, not an index: **why it matters → what the space is → how it works → who does it → where the answer / openings are.** Each part sets up the next. Open with a short *"how to read this"* preface that states the through-line; let each unit set up the following one (the progressive thread — chapter N's problem is partly solved by chapter N+1). The last beat adapts to genre (table above).
2. **Parallel structure for every unit — same beats, same order.** Pick the loop from the genre table and hold it *identically* across every unit. The reader learns the shape once, then scans the rest fast. **Uneven sections are the #1 readability killer.**
3. **A fixed "readout" per unit, genre-matched** (table above). End every unit with the same compact block: viability-strip for options, `> Спросить дословно: "…"` + model answer for knowledge, next-action for decision. Same shape every time → scannable and comparable at a glance; tells the reader what to *do* with the unit.
4. **A diagram where a process is the point.** When a unit is "what happens / what gets automated / how data flows / how ideas connect", show a step flow or block-schema with a consistent legend (tool vs human, or the pipeline stages), not prose.
5. **A ranked summary near the end.** One table over ALL units on the same axes, sorted by the reader's real question. Options rank "easiest / most valuable first"; a knowledge doc ranks its **question-bank** — what to master first. This payoff makes the map usable.
6. **Lead concrete; description over prescription.** Lead with the concrete and replicable; treat giant/abstract examples as proof of demand, not the hero. Let facts point to the opening rather than ordering the reader around — unless a recommendation is the explicit purpose.
7. **Always show boundaries — this is a beat, not a caveat.** A "where it does NOT work" per unit. In the knowledge genre it's the mandatory *bad-practices / where it breaks* section, written as `cause → effect → failure` chains ("chunking без overlap → обрывки без контекста → модель додумывает → галлюцинации"), never as vague hedging. Honesty here is what makes the rest trustworthy.
8. **Plain and sourced.** Sober tone (no "revolutionary"), explain each unavoidable term once, every number carries a source. If shared outside, depersonalize — strip the requester's identity, reframe for the generic actor.
9. **Cross-link related beats — a long doc is a web, not a list.** When one unit depends on, extends, or contradicts another, link it inline ("see 3.7", "as in the caching chapter"). Without cross-refs a multi-chapter document reads as isolated silos; with them it becomes one fabric the reader can navigate. A knowledge doc especially: MCP → CLI-agents → context-engineering only *teach* when the reader sees the threads between them.

### Genre pattern: Mentor cut (teaching guide)

For a **mentor-to-intern guide**, every chapter/unit follows one fixed loop:

1. **The Setup** — the naive version with working code. Show the beginner approach that *works perfectly — until it doesn't*. Give a concrete code example (real syntax, not pseudocode). The reader must feel "yes, I'd do that" before you break it.
2. **The Problem** — a *named* failure mode with real stakes (e.g. "Sticky Sessions," "Thundering Herd," "N+1 Problem"). Describe the exact production scenario where the naive approach collapses. Mention concrete consequences: lost data, 3 AM pages, support queue filling up. Contrast bad fixes (workarounds that create new problems) with the real one.
3. **The Real Fix** — the solution. Include a **diagram** (block-schema, flow, or ring — not prose when a process is the point). Show code for the fix, not just description.
4. **Why this comes first** — the ordering rationale: what prerequisite does this chapter establish for the next one? End with a **"common mistake I made as a junior"** confession — personal, concrete, the exact anti-pattern the mentor actually committed.

Additional constraints:
- A **Preface: How to Read This Guide** that states the through-line and the progression logic (each chapter introduces a problem the next partially solves)
- Cross-chapter progression: chapter N's problem is partly solved by chapter N+1
- Diagrams use a consistent legend
- Every chapter ends with a "where it does NOT work" / failure-mode section — written as `cause → effect → failure` chains, never vague hedging
- Lead concrete, not abstract; description over prescription

### Genre pattern: Field-textbook cut (interview prep)

For a **field textbook** aimed at interview preparation, every chapter/unit follows a six-beat loop:

1. **Боль (pain)** — the problem that forced invention of this technology. Answer: *why does this exist?* Not a definition. The reader must feel the pain before seeing the cure.
2. **Как это работает (how it works)** — mechanics. The minimum a candidate must understand to reason about tradeoffs.
3. **Блок-схема (diagram)** — an ASCII-art or Mermaid block-schema replacing a thousand words.
4. **Хорошие практики (good practices)** — what people who already got burned do. Terse, in checklist form.
5. **Плохие практики / где ломается (bad practices / failure modes)** — concrete `cause → effect → failure` chains. Every bad practice named as a *predicate* that produces a *symptom*.
6. **Что спросят на интервью (interview question)** — verbatim `> Q:` callout with a model answer that *explains the why*, not restates the definition.

Additional constraints:
- **Cross-refs as web:** `см. N.N` references bind chapters into one fabric — no chapter is an island
- **UNCONFIRMED annotations** on claims not verified from primary sources — honesty about what is and isn't known
- **Verbatim interview prompts:** `> Спросить дословно: "…"` — exact phrasing the interviewer might use
- A preface that names the role being targeted and states the reading strategy
- A ranked **question-bank** near the end — what to master first, sorted by interview weight

### Report anti-patterns

The fast way to lose the reader: a definition-first opening (what before why); one chapter following loop A and the next loop B; boundaries collapsed into a hand-wavy "your mileage may vary"; a wall of prose where a diagram belonged; chapters that never reference each other; a giant flagship example presented as the hero instead of as proof.
