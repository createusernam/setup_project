...
3. Run `setup-pipeline status` from the project root. If the command is missing, report a setup
...
4. Read the phase from `.pipeline-state.json` and run `setup-preflight <phase> <project-root>`.
   Preflight is read-only. A nonzero result means BLOCKED, not that the project is broken.
